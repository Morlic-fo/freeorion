...
def generate_production_orders():
    ...
    if (currentTurn in [1, 4]) and ((productionQueue.totalSpent < totalPP) or (len(productionQueue) <= 3)):
        bestDesignID, bestDesign, buildChoices = get_best_ship_info(
            EnumsAI.AIPriorityType.PRIORITY_PRODUCTION_EXPLORATION)
        ...
        if bestDesignID:
            for scout_count in range(init_scouts):
                foAI.foAIstate.production_queue_manager.enqueue_item(SHIP, bestDesignID, buildChoices[0],
                                                                     PRIORITY_SHIP_SCOUT)
    ...
    if not homeworld:
        ...
        for bldg in homeworld.buildingIDs:
            ...
            print "%8s | %20s | type:%20s | tags:%20s | specials: %20s | owner:%d " % (
                bldg, thisObj.name, "_".join(thisObj.buildingTypeName.split("_")[-2:])[:20], tags, specials, thisObj.owner )
        print
        ...
        for bldTID in empire.availableBuildingTypes:
            try:
                ...
                if fo.getBuildingType(bldTID) is None:
                    print "For empire %d, 'available Building Type ID' %s returns Nonetype from fo.getBuildingType(bldTID)" % (
        ...
        if possibleBuildingTypeIDs:
            ...
            for buildingTypeID in possibleBuildingTypeIDs:
                ...
                print "    " + str(buildingType.name) + " cost: " + str(
                    buildingType.productionCost(empire.empireID, homeworld.id)) + " time: " + str(
                    buildingType.productionTime(empire.empireID, homeworld.id))

            possibleBuildingTypes = [fo.getBuildingType(buildingTypeID) and fo.getBuildingType(buildingTypeID).name for
                                     buildingTypeID in
                                     possibleBuildingTypeIDs]  #makes sure is not None before getting name
            ...
            if (totalPP > 40 or (
                        (currentTurn > 40 ) and (ColonisationAI.empire_status.get('industrialists', 0) >= 20 ))) and (
                        "BLD_INDUSTRY_CENTER" in possibleBuildingTypes) and (
                        "BLD_INDUSTRY_CENTER" not in (capitalBldgs + queuedBldgNames)) and (bldgExpense < bldgRatio * totalPP):
            ...
            if ("BLD_SHIPYARD_BASE" in possibleBuildingTypes) and (
                        "BLD_SHIPYARD_BASE" not in (capitalBldgs + queuedBldgNames)):
                try:
            ...
            for bldName in ["BLD_SHIPYARD_ORG_ORB_INC"]:
                if (bldName in possibleBuildingTypes) and (bldName not in (capitalBldgs + queuedBldgNames)) and (
                            bldgExpense < bldgRatio * totalPP):
                    try:
                        res = foAI.foAIstate.production_queue_manager.enqueue_item(BUILDING, bldName, homeworld.id,
            ...
            if ("BLD_IMPERIAL_PALACE" in possibleBuildingTypes) and (
                        "BLD_IMPERIAL_PALACE" not in (capitalBldgs + queuedBldgNames)):
            ...
            if ("BLD_NEUTRONIUM_SYNTH" in possibleBuildingTypes) and (
                        "BLD_NEUTRONIUM_SYNTH" not in (capitalBldgs + queuedBldgNames)):
    ...
    if ( (currentTurn % aggrIndex) == 0) and foAI.foAIstate.aggression < fo.aggression.maniacal:
        ...
        targetOrbitals = min(int(((currentTurn + 4) / ( 8.0 * aggrIndex ** 1.5)) ** 0.8),
                             fo.aggression.maniacal - aggrIndex)
        print "Orbital Defense Check -- target Defense Orbitals: ", targetOrbitals
        for element in productionQueue:
            if ( element.buildType == EnumsAI.AIEmpireProductionTypes.BT_SHIP) and (
                        foAI.foAIstate.get_ship_role(element.designID) == EnumsAI.AIShipRoleType.SHIP_ROLE_BASE_DEFENSE):
        ...
        print "Queued Defenses:", [( ppstring(PlanetUtilsAI.sys_name_ids([sysID])), num) for sysID, num in
                                   queuedDefenses.items()]
        for sysID in ColonisationAI.empire_species_systems:
            ...
            for fid in fleetsHere:
                if foAI.foAIstate.get_fleet_role(fid) == EnumsAI.AIFleetMissionType.FLEET_MISSION_ORBITAL_DEFENSE:
                    print "Found %d existing Orbital Defenses in %s :" % (
                        ...
                        ppstring(PlanetUtilsAI.sys_name_ids([sysID])))
            ...
            if sysOrbitalDefenses[sysID] < targetOrbitals:
                ...
                for pid in ColonisationAI.empire_species_systems.get(sysID, {}).get('pids', []):
                    bestDesignID, colDesign, buildChoices = get_best_ship_info(
                    ...
                    for i in xrange(0, numNeeded):
                        retval = foAI.foAIstate.production_queue_manager.enqueue_item(SHIP, bestDesignID, pid,
                                                                                      PRIORITY_ORBITAL_DEFENSE)
                    print "queueing %d Orbital Defenses at %s" % (
                        numNeeded, ppstring(PlanetUtilsAI.planet_name_ids([pid])))
                    if retval != 0:
                        cost, time = empire.productionCostAndTime(productionQueue[productionQueue.size - 1])
    ...
    for specName in ColonisationAI.empire_colonizers:
        if (len(ColonisationAI.empire_colonizers[specName]) == 0) and (
                    specName in ColonisationAI.empire_species):  #not enough current shipyards for this species#TODO: also allow orbital incubators and/or asteroid ships
            for pID in ColonisationAI.empire_species.get(specName,
                ...
                planetList = zip(map(universe.getPlanet, ColonisationAI.empire_species[specName]),
                                 ColonisationAI.empire_species[specName])
                pops = sorted([(planet.currentMeterValue(fo.meterType.population), pID) for planet, pID in planetList if
                ...
                if pids:
                    buildLoc = pids[-1]
                    res = foAI.foAIstate.production_queue_manager.enqueue_item(BUILDING, "BLD_SHIPYARD_BASE", buildLoc,
                                                                               PRIORITY_BUILDING_LOW)
                    print "Enqueueing BLD_SHIPYARD_BASE at planet %d (%s) for colonizer species %s, with result %d" % (
                        buildLoc, universe.getPlanet(buildLoc).name, specName, res)
                    if res:
    ...
    for pid in ColonisationAI.empire_species.get("SP_ACIREMA", []):
        ...
        res = foAI.foAIstate.production_queue_manager.enqueue_item(BUILDING, "BLD_SHIPYARD_BASE", pid,
                                                                   PRIORITY_BUILDING_LOW)
        print "Enqueueing BLD_SHIPYARD_BASE at planet %d (%s) for Acirema, with result %d" % (
    ...
    for pid, rating in ColonisationAI.pilot_ratings.items():
        ...
        res = foAI.foAIstate.production_queue_manager.enqueue_item(BUILDING, "BLD_SHIPYARD_BASE", pid,
                                                                   PRIORITY_BUILDING_LOW)
        print "Enqueueing BLD_SHIPYARD_BASE at planet %d (%s) for top pilot, with result %d" % (
            pid, universe.getPlanet(pid).name, res)
        if res:
            queuedShipyardLocs.append(pid)
    ...
    for bldName in ["BLD_SHIPYARD_ENRG_COMP"]:
        if empire.buildingTypeAvailable(bldName):
            ...
            for pid in bh_pilots + blue_pilots:
                ...
                if not (
                            this_planet and this_planet.speciesName in ColonisationAI.empire_ship_builders):  #TODO: also check that not already one for this spec in this sys
                    continue
                ...
                if pid not in queuedBldLocs and bldType.canBeProduced(empire.empireID, pid):
                    res = foAI.foAIstate.production_queue_manager.enqueue_item(BUILDING, bldName, pid,
                                                                               PRIORITY_BUILDING_LOW)
                    print "Enqueueing %s at planet %s, with result %d" % (bldName, universe.getPlanet(pid).name, res)
                    if _CHAT_DEBUG:
                        chat_human(
                            "Enqueueing %s at planet %s, with result %d" % (bldName, universe.getPlanet(pid), res))
                    if res:
    ...
    if empire.buildingTypeAvailable(bldName) and (bldgExpense < bldgRatio * totalPP) and (
                    totalPP > 50 or currentTurn > 80 ):
        bldType = fo.getBuildingType(bldName)
        for sys_id in enrgyShipyardLocs:  #Todo ensure only one or 2 per sys
            for pid in enrgyShipyardLocs[sys_id][:2]:
                if pid not in queuedShipyardLocs and bldType.canBeProduced(empire.empireID,
                                                                           pid):  #TODO: verify that canBeProduced() checks for prexistence of a barring building
                    res = foAI.foAIstate.production_queue_manager.enqueue_item(BUILDING, bldName, pid,
                                                                               PRIORITY_BUILDING_LOW)
                    print "Enqueueing %s at planet %d (%s) , with result %d" % (
                        bldName, pid, universe.getPlanet(pid).name, res)
                    if res:
                        queuedShipyardLocs.append(pid)
                        cost, time = empire.productionCostAndTime(productionQueue[productionQueue.size - 1])
                        bldgExpense += cost / time  # productionQueue[productionQueue.size -1].blocksize *
        ...
        if not queuedBldLocs:
            ...
            for pid in list(AIstate.popCtrIDs) + list(AIstate.outpostIDs):
                ...
                if planet.size == fo.planetSize.asteroids and sysID in ColonisationAI.empire_species_systems:
                    ...
                    if (pid in queuedBldLocs) or (
                                bldName in [universe.getObject(bldg).buildingTypeName for bldg in planet.buildingIDs]):
            ...
            for rating, pid, sysID in topPilotLocs:
                if sysID not in yardLocs:
                    ...
                    for pid, rating in top_pilot_systems[sysID]:
                        if pid not in queuedShipyardLocs:  #will catch it later if shipyard already present
                            needYard[sysID] = pid
            if ( not yardLocs ) and len(asteroidYards.values()) <= int(
                            currentTurn / 50):  # not yet building & not enough current locs, find a location to build one
                    ...
                    for thisSpec, pid in builderSystems[sysID]:
                        if thisSpec in ColonisationAI.empire_colonizers:
                            ...
                            if pid in (ColonisationAI.empire_ship_builders.get(thisSpec, []) + queuedShipyardLocs):
                                ...
                                needYard[sysID] = pid
                yardLocs.extend(
                    (colonizerLocChoices + builderLocChoices)[:1])  #add at most one of these non top pilot locs
            ...
            for sysID in yardLocs:  #build at most 2 new asteroid yards at a time
                ...
                if sysID in needYard:
                    ...
                    if ShipYardType.canBeProduced(empire.empireID, pid2):
                        res = foAI.foAIstate.production_queue_manager.enqueue_item(BUILDING, "BLD_SHIPYARD_BASE", pid2,
                                                                                   PRIORITY_BUILDING_LOW)
                        print "Enqueueing %s at planet %d (%s) to go with Aseroid Processor , with result %d" % (
                            "BLD_SHIPYARD_BASE", pid2, universe.getPlanet(pid2).name, res)
                        if res:
                ...
                if pid not in queuedBldLocs and bldType.canBeProduced(empire.empireID, pid):
                    res = foAI.foAIstate.production_queue_manager.enqueue_item(BUILDING, bldName, pid,
                                                                               PRIORITY_BUILDING_LOW)
                    print "Enqueueing %s at planet %d (%s) , with result %d on turn %d" % (
                        bldName, pid, universe.getPlanet(pid).name, res, currentTurn)
                    if res:
                        newYardCount += 1
                        queuedBldLocs.append(pid)
                        cost, time = empire.productionCostAndTime(productionQueue[productionQueue.size - 1])
    ...
    if empire.buildingTypeAvailable(bldName) and foAI.foAIstate.aggression > fo.aggression.beginner:
        queuedBldLocs = [element.locationID for element in productionQueue if (element.name == bldName)]
        bldType = fo.getBuildingType(bldName)
        for pid in list(AIstate.popCtrIDs) + list(
                AIstate.outpostIDs):  #TODO: check to ensure that a resource center exists in system, or GGG would be wasted
            if pid not in queuedBldLocs and bldType.canBeProduced(empire.empireID,
                                                                  pid):  #TODO: verify that canBeProduced() checks for prexistence of a barring building
                ...
                if thisPlanet.systemID in ColonisationAI.empire_species_systems:
                    ...
                    for opid in system.planetIDs:
                        ...
                        if otherPlanet.size == fo.planetSize.gasGiant:
                            GGList.append(opid)
                        if opid != pid and otherPlanet.owner == empire.empireID and (
                                    EnumsAI.AIFocusType.FOCUS_INDUSTRY in list(otherPlanet.availableFoci) + [
                                    otherPlanet.focus]):
                            canUseGGG = True
                ...
                if not homeworld:
                    ...
                    for sysID in best_locs:  #want to build close to capitol for defense
                        ...
                        try:
                            ...
                            pass
                    useSys = ([(-1, -1)] + sorted([(dist, sysID) for sysID, dist in distanceMap.items()]))[:2][-1][
                        -1]  # kinda messy, but ensures a value
                if useSys != -1:
                    try:
                        useLoc = AIstate.colonizedSystems[useSys][0]
                        res = foAI.foAIstate.production_queue_manager.enqueue_item(BUILDING, bldName, useLoc,
                                                                                   PRIORITY_BUILDING_HIGH)
                        print "Enqueueing %s at planet %d (%s) , with result %d" % (
                            bldName, useLoc, universe.getPlanet(useLoc).name, res)
                        if res:
                            cost, time = empire.productionCostAndTime(productionQueue[productionQueue.size - 1])
                            bldgExpense += cost / time  # productionQueue[productionQueue.size -1].blocksize *
                    except:
    ...
    if ( ( empire.buildingTypeAvailable(bldName) ) and (foAI.foAIstate.aggression > fo.aggression.typical) and
        ...
        queuedBldLocs = [element.locationID for element in productionQueue if
                         (element.name == bldName)]  #TODO: check that queued locs or already built one are at red stars
        if not bh_pilots and len(queuedBldLocs) == 0 and (red_pilots or not alreadyGotOne):  #
            ...
            if use_loc is not None:
                ...
                try:
                    res = foAI.foAIstate.production_queue_manager.enqueue_item(BUILDING, bldName, use_loc,
                                                                               PRIORITY_BUILDING_LOW)
    ...
    if empire.buildingTypeAvailable(bldName) and foAI.foAIstate.aggression > fo.aggression.cautious:
        ...
        if (len(AIstate.empireStars.get(fo.starType.blackHole, [])) > 0) and len(
                queuedBldLocs) == 0 and not alreadyGotOne:  #
            if not homeworld:
    ...
    if empire.buildingTypeAvailable(bldName):
        ...
        if len(queuedLocs) == 0 and homeworld and not alreadyGotOne:  #
            try:
                res = foAI.foAIstate.production_queue_manager.enqueue_item(BUILDING, bldName, capitolID,
                                                                           PRIORITY_BUILDING_HIGH)
                print "Enqueueing %s at planet %d (%s) , with result %d" % (
                    bldName, capitolID, universe.getPlanet(capitolID).name, res)
            except:
                pass
    ...
    if (empire.buildingTypeAvailable(bldName) and
        ...
        for pid in list(AIstate.popCtrIDs) + list(AIstate.outpostIDs):
            ...
            if ( planet and (( planet.systemID in AIstate.empireStars.get(fo.starType.neutron, []) and
                                   (bldName in [bld.buildingTypeName for bld in
                                                map(universe.getObject, planet.buildingIDs)]) ) or
                                 ("BLD_NEUTRONIUM_SYNTH" in [bld.buildingTypeName for bld in
                                                             map(universe.getObject, planet.buildingIDs)])
            )):
    ...
    for fid in FleetUtilsAI.get_empire_fleet_ids_by_role(EnumsAI.AIFleetMissionType.FLEET_MISSION_COLONISATION):
        ...
        for shipID in fleet.shipIDs:
            ...
            if thisShip and (foAI.foAIstate.get_ship_role(
                    thisShip.design.id) == EnumsAI.AIShipRoleType.SHIP_ROLE_CIVILIAN_COLONISATION):
    ...
    for pid in AIstate.popCtrIDs:
        ...
        builtCamp = False
        thisSpec = planet.speciesName
        safetyMarginMet = ((thisSpec in ColonisationAI.empire_colonizers and (
            len(ColonisationAI.empire_species.get(thisSpec, []) + colonyShipMap.get(thisSpec, [])) >= 2 )) or (cPop >= 50 ))
        has_camp = False
                ...
                if popDisqualified:
                    ...
                    if verboseCamp:
                        print "Conc Camp disqualified at %s due to safety margin; species %s, colonizing planets %s, with %d colony ships" % (
                            ...
                            len(colonyShipMap.get(planet.speciesName, [])))
            if has_camp:
                res = fo.issueScrapOrder(bldg)
                print "Tried scrapping %s at planet %s, got result %d" % (bldName, planet.name, res)
        elif foAI.foAIstate.aggression > fo.aggression.typical and canBuildCamp and (tPop >= 36) and not has_camp:
            if (planet.focus == EnumsAI.AIFocusType.FOCUS_GROWTH) or ("COMPUTRONIUM_SPECIAL" in planet.specials) or (
                        pid == capitolID):
                continue
    ...
    if empire.buildingTypeAvailable(bldName):
        ...
        for pid in list(AIstate.popCtrIDs) + list(AIstate.outpostIDs):
            planet = universe.getPlanet(pid)
            if planet:
                if ( pid in queuedLocs ) or (
                            bldName in [bld.buildingTypeName for bld in map(universe.getObject, planet.buildingIDs)] ):
        ...
        for sysID in AIstate.colonizedSystems:
            ...
            for troops, loc in sorted(buildLocs):
                planet = universe.getPlanet(loc)
                res = foAI.foAIstate.production_queue_manager.enqueue_item(BUILDING, bldName, loc,
                                                                           PRIORITY_BUILDING_LOW)
    ...
    if empire.buildingTypeAvailable(bldName):
        ...
        print "Considering building %s, found current and queued systems %s" % (
            bldName, ppstring(PlanetUtilsAI.sys_name_ids(cur_drydoc_sys.union(queued_sys))))
    ...
    for entry in colony_bldg_entries:
        ...
        if not (bldType and bldType.canBeEnqueued(empire.empireID, pid)):
            continue
        res = foAI.foAIstate.production_queue_manager.enqueue_item(BUILDING, bldName, pid, PRIORITY_BUILDING_HIGH)
        print "Enqueueing %s at planet %d (%s) , with result %d" % (bldName, pid, planet.name, res)
        if res:
            break
        else:
    ...
    if False:
        ...
        if homeworld:
            for shipDesignID in empire.availableShipDesigns:
                ...
                print "    " + str(shipDesign.name(True)) + " cost:" + str(
                    shipDesign.productionCost(empire.empireID, homeworld.id)) + " time:" + str(
                    shipDesign.productionTime(empire.empireID, homeworld.id))
    ...
    for queue_index in range(len(productionQueue)):
        ...
        print "    " + blockStr + element.name + " requiring " + str(
            element.turnsLeft) + " more turns; alloc: %.2f PP" % element.allocation + " with cum. progress of %.1f" % element.progress + " being built at " + universe.getObject(
            element.locationID).name
        if element.turnsLeft == -1:
            if element.locationID not in AIstate.popCtrIDs + AIstate.outpostIDs:
                ...
                print "element %s will never be completed as stands and location %d no longer owned; could consider deleting from queue " % (
                    element.name, element.locationID)  #TODO:
            else:
    ...
    print "Trooper Status turn %d: %d total, with %d unassigned. %d queued, compared to %d total Military Attack Ships" % (
        currentTurn, nTroopTot,
        nAvailTroopTot, queuedTroopShips, nMilitaryTot)
    if (capitolID is not None and ((currentTurn >= 40) or can_prioritize_troops) and foAI.foAIstate.systemStatus.get(
            capitolSysID, {}).get('fleetThreat', 0) == 0 and
        ...
        if buildChoices is not None and len(buildChoices) > 0:
            ...
            troopersNeededForcing = max(0, int(
                min(0.99 + (currentTurn / 20.0 - nAvailTroopTot) / max(2, prodTime - 1), nMilitaryTot / 3 - nTroopTot)))
            ...
            if troopersNeededForcing > 0 and totalPP > 3 * perTurnCost * queuedTroopShips and foAI.foAIstate.aggression >= fo.aggression.typical:
                for i in xrange(0, numShips):
                    retval = foAI.foAIstate.production_queue_manager.enqueue_item(SHIP, bestDesignID, loc,
                                                                                  PRIORITY_SHIP_TROOPS)
                if retval != 0:
                    print "forcing %d new ship(s) to production queue: %s; per turn production cost %.1f" % (
                        numShips, bestDesign.name(True), numShips * perTurnCost)
                    print
    ...
    topscore = -1
    numColonyFleets = len(FleetUtilsAI.get_empire_fleet_ids_by_role(
    ...
    numOutpostFleets = len(FleetUtilsAI.get_empire_fleet_ids_by_role(
    ...
    for ID, score in sortedPriorities:
        ...
        if ID != EnumsAI.AIPriorityType.PRIORITY_PRODUCTION_BUILDINGS:
            if ( ID == EnumsAI.AIPriorityType.PRIORITY_PRODUCTION_COLONISATION ) and (
                        totColonyFleets < maxColonyFleets) and (colonyBuildChoices is not None) and len(colonyBuildChoices) > 0:
                filteredPriorities[ID] = score
            elif ( ID == EnumsAI.AIPriorityType.PRIORITY_PRODUCTION_OUTPOST ) and (
                        totOutpostFleets < maxOutpostFleets ):
                filteredPriorities[ID] = score
    ...
    availablePP = dict([(tuple(el.key()), el.data() ) for el in
                        empire.planetsWithAvailablePP])  #keys are sets of ints; data is doubles
    allocatedPP = dict([(tuple(el.key()), el.data() ) for el in
    ...
    for pSet in availablePP:
        print "\t%s\t%.2f" % (
            ppstring(PlanetUtilsAI.sys_name_ids(set(PlanetUtilsAI.get_systems(pSet)))), availablePP[pSet])
    print "\nallocatedPP ( <systems> : pp ):"
    for pSet in allocatedPP:
        print "\t%s\t%.2f" % (
            ppstring(PlanetUtilsAI.sys_name_ids(set(PlanetUtilsAI.get_systems(pSet)))), allocatedPP[pSet])
    ...
    for pSet in planetsWithWastedPP:
        ...
        print "%.2f PP remaining in system group: %s" % (
            availPP, ppstring(PlanetUtilsAI.sys_name_ids(set(PlanetUtilsAI.get_systems(pSet)))))
        ...
        for pid, (score, this_spec) in foAI.foAIstate.colonisablePlanetIDs.items():
            colonyBuildChoices.extend(
                int(math.ceil(score)) * [pid2 for pid2 in speciesMap.get(this_spec, []) if pid2 in pSet])
        ...
        for priority in list(localPriorities):
            ...
            print "bestShips[%s] = %s \t locs are %s from %s" % (
                EnumsAI.AIPriorityNames[priority], bestDesign.name(False), buildChoices, pSet)
        ...
        loopCount = 0
        while (availPP > 0) and (loopCount < max(100, currentTurn)) and (
                    priorityChoices != [] ):  #make sure don't get stuck in some nonbreaking loop like if all shipyards captured
            loopCount += 1
            ...
            if makingColonyShip:
                ...
                if bestDesign is None:
                    print "Error: problem with milBuildChoices; with selector (%s) chose loc (%s), bestDesignID (%s), bestDesign (None) from milBuildChoices: %s" % (
                        selector, loc, bestDesignID, milBuildChoices)
                    continue
            ...
            numShips = 1
            perTurnCost = (
                float(bestDesign.productionCost(empire.empireID, loc)) / bestDesign.productionTime(empire.empireID, loc))
                ...
                if ratingRatio < 0.1:
                    ...
                    if locPlanet:
                        ...
                        print "Building mil ship at loc %d (%s) with %s pilot Rating: %.1f; ratio to empire best is %.1f" % (
                            loc, pname, qualifier, thisRating, ratingRatio)
            ...
            if retval != 0:
                print "adding %d new ship(s) at location %s to production queue: %s; per turn production cost %.1f" % (
                    numShips, ppstring(PlanetUtilsAI.planet_name_ids([loc])), bestDesign.name(True), perTurnCost)
                print
                availPP -= perTurnCost
                if makingColonyShip:
