...
class ShipyardManager(object):  # TODO: Inherit from base building class...
    """The generic base class for any shipyard manager.

    This class provides a framework to find and rate candidates of planets where shipyards may be built.
    """
    name = ""
    minimum_spacing = 5  # minimum distance to the next "equivalent" shipyard of the same type
    production_cost = 99999
    production_time = 99999
    prereqs = []  # prerequities that need to be present on the same planet
    system_prereqs = []  # prerequisites that need to be present in the same system
    unlocked_hulls = []  # a list of hulls unlocked by the shipyard
    unlocked_parts = []  # a list of parts unlocked by the shipyard
    ...
    def _get_candidate_list(self):
        """Get a list of candidate locations for this shipyards.

        Candidates are planets which unlock access to the highest rated ships by building this yard.
        :return: candidates
        :rtype: list
        """
    ...
    def _get_rating_improvements(self, pid):
        """Calculate the rating improvement obtained by building this shipyard (i.e. unlocking parts/hulls)."""
    ...
    def _get_cheapest_candidates(candidate_list):
        """Get the cheapest candidates within the list.

        :param candidate_list: list with ShipyardLocationCandidate s
        :type candidate_list: list
        :return: List of cheapest ShipyardLocationCandidate s
        :rtype: list
        """
    ...
    def _get_shipbuilding_planets(planet_list):
        """Get the planets on which the species can build ships.

        :param planet_list: list of planet_ids to query
        :type planet_list: list
        :return: list of planet_ids where the species can build ships
        :rtype: list
        """
        universe = fo.getUniverse()
        locs = []
...
class AsteroidShipyardManager(BasicShipyardManager):
    ...
    Needs asteroid belt, is system-wide"""
...
class ShipyardLocationCandidate(object):
    ...
    def _calc_minimum_distance_to_yards(self, yard_locs):
        ...
        :param yard_locs: Yards to calculate the distance to
        """
        universe = fo.getUniverse()
    ...
    def _calc_missing_prereqs(self, prereqs):
        ...
        :param prereqs: List of prereqs to be considered"""
        missing_prereqs = []
        for prereq in prereqs:
            if self.pid not in BuildingsAI.bld_cache.existing_buildings.get(prereq, []):
        ...
        if self.system_prereqs:
            ...
            for prereq in self.system_prereqs:
                existing_locs = BuildingsAI.bld_cache.existing_buildings.get(self.this_shipyard, [])
                if not any(pid in existing_locs for pid in planet_ids):
                    missing_prereqs.append(prereq)
        return missing_prereqs

    def _calc_total_pp_cost(self):
        """Calculate the total productionPoint cost for this shipyard including prerequisites."""
        empire_id = fo.empireID()
