...
def get_best_ship_info(priority, loc=None):
    """ Returns 4 item tuple: designID, design, buildLocList, rating."""
    if loc is None:
    ...
    if priority not in _design_cache:
        # must be missing a Shipyard or other orbital (or missing tech)
        return None, None, None, None
    ...
    for design_stats in best_designs:
        ...
        # apparently can't build for this priority within the desired planet group
        return None, None, None, None
    valid_locs = [item[1] for item in best_designs
                  if item[0] == top_rating and item[2] == top_id]
...
def generate_production_orders():
    ...
    _, _, colony_build_choices, _ = get_best_ship_info(PriorityType.PRODUCTION_COLONISATION)
    ...
    for planet_set in planets_with_wasted_pp:
        ...
        best_design_id, best_design, build_choices, _ = get_best_ship_info(
                PriorityType.PRODUCTION_COLONISATION, list(planet_set))
        species_map = {}
        ...
        for priority in list(local_priorities):
            if priority == PriorityType.PRODUCTION_MILITARY:
                ...
                best_design_id, best_design, build_choices, _ = get_best_ship_info(priority, list(planet_set))
        ...
        while (avail_pp > 0) and (loop_count < max(100, current_turn)) and (priority_choices != []):
            ...
            if making_colony_ship:
                ...
                if best_design is None:
                    warn("problem with mil_build_choices; with selector (%s)"
                         " chose loc (%s), best_design_id (%s),"
                         " best_design (None) from mil_build_choices: %s" % (
...
def _build_orbital_defenses():
    ...
    allowed_defense_allocation = foAI.foAIstate.character.max_defense_portion() * empire.productionPoints
    ...
    # account already enqueued orbital defenses
    queued_defenses = {}
    for element in production_queue:
        if (element.buildType != EmpireProductionTypes.BT_SHIP or
                foAI.foAIstate.get_ship_role(element.designID) != ShipRoleType.BASE_DEFENSE):
            continue

        sys_id = universe.getPlanet(element.locationID).systemID
    ...
    for sys_id, pids in state.get_empire_planets_by_system(include_outposts=False).items():
        ...
        for pid in pids:
            best_design_id, _, _, _ = get_best_ship_info(PriorityType.PRODUCTION_ORBITAL_DEFENSE, pid)
